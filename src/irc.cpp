...
// Copyright (c) 2011-2012 CashhCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("CashhCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CashhCoinIRCTEST0\r");
            Send(hSocket, "WHO #CashhCoinIRCTEST0\r");
        } else {
            // randomly join #CashhCoinIRC00-#CashhCoinIRC99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #CashhCoinIRC%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CashhCoinIRC%02d\r", channel_number).c_str());
        }
